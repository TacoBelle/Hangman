...
function Word() {
	...
	this.getWord = function() {
		currentWord = this.wordList[Math.floor(Math.random() * this.wordList.length)];
	...
	//write to HTML
	//CAMBIAR CURRENTWORD YA QUE SE CANCELO Y ACTUALMENTE SE PASA COMO PARAMETRO DESDE MAIN.
		...
		if (guessIsRight == true)
			{
			   ...
			   console.log("El jugador acerto la letra" + currentWordLetter + " en la palabra " + a + "por lo que se suman puntos");
			   console.log("El valor de plaholder es" + placeholder + " y la palabra es " + a );
			   if(placeholder.join("") == a.join(""))
			   {
			   		console.log("ganaste,");
			   }
			...
			else 
			{
		       newPlayer.setLives();
                   hangMan.drawHangman();
		       this.playerInputEl.value = "";
		       console.log("El jugador no acerto la letra" + currentWordLetter + " en la palabra " + a + "por lo que pierde vidas. Sus vidas totales al momento son " + newPlayer.getLives());
			}
